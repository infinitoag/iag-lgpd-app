(function (window, document) {
  if (window.__iagLGPDAppLoaded) return;
  window.__iagLGPDAppLoaded = true;

  var COOKIE_NAME = 'iag_lgpd_consent';
  var COOKIE_EXPIRY_DAYS = 30;
  var DEFAULTS = {
    banner_type: 'bottom_bar',
    message: 'Este site utiliza cookies para melhorar sua experiência. Ao continuar, você aceita o uso de cookies.',
    banner_bg_color: '#FFF',
    banner_text_color: '#000',
    btn_bg_color: '#000',
    btn_text_color: '#FFF',
    show_reject_button: false,
    reject_redirect_url: '',
    custom_css: '',
    policy_url: '',
    tags_before: [],
    tags_after: [],
    useConsentModeV2: false,
    gtm: '',
  };

  function safeGet(arg, key) {
    return arg && Object.prototype.hasOwnProperty.call(arg, key) ? arg[key] : DEFAULTS[key];
  }

  function setCookie(name, value, days) {
    var expires = '';
    if (typeof days === 'number') {
      var date = new Date();
      date.setTime(date.getTime() + days * 24 * 60 * 60 * 1000);
      expires = '; expires=' + date.toUTCString();
    }
    document.cookie = name + '=' + encodeURIComponent(value) + expires + '; path=/; SameSite=Lax';
  }

  function getCookie(name) {
    var pairs = document.cookie ? document.cookie.split('; ') : [];
    for (var i = 0; i < pairs.length; i += 1) {
      var parts = pairs[i].split('=');
      if (parts[0] === name) {
        return decodeURIComponent(parts.slice(1).join('='));
      }
    }
    return null;
  }

  function isBannerVisible() {
    return getCookie(COOKIE_NAME) === null;
  }

  function appendHTML(html, location) {
    var wrapper = document.createElement('div');
    wrapper.innerHTML = html;
    Array.prototype.slice.call(wrapper.childNodes).forEach(function (node) {
      if (node.nodeName === 'SCRIPT') {
        var script = document.createElement('script');
        for (var i = 0; i < node.attributes.length; i += 1) {
          var attr = node.attributes[i];
          script.setAttribute(attr.name, attr.value);
        }
        script.text = node.textContent;
        script.async = false;
        location.appendChild(script);
      } else {
        location.appendChild(node);
      }
    });
  }

  function injectTags(tags) {
    if (!Array.isArray(tags) || tags.length === 0) return;
    var target = document.body || document.head || document.documentElement;
    tags.forEach(function (tagString) {
      if (typeof tagString !== 'string' || !tagString.trim()) return;
      appendHTML(tagString, target);
    });
  }

  function injectStyles(styles) {
    if (typeof styles !== 'string' || !styles.trim()) return;
    var style = document.createElement('style');
    style.type = 'text/css';
    style.appendChild(document.createTextNode(styles));
    (document.head || document.documentElement).appendChild(style);
  }

  function injectGTM(gtmId, useConsentModeV2) {
    if (!gtmId || typeof gtmId !== 'string') return;
    var head = document.head || document.getElementsByTagName('head')[0] || document.documentElement;

    if (useConsentModeV2) {
      var configScript = document.createElement('script');
      configScript.type = 'text/javascript';
      configScript.text = "window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('consent','default',{ad_storage:'denied',analytics_storage:'denied',ad_user_data:'denied',ad_personalization:'denied',functionality_storage:'denied',security_storage:'denied'});gtag('js', new Date());";
      head.appendChild(configScript);
    }

    var gtmScript = document.createElement('script');
    gtmScript.async = true;
    gtmScript.src = 'https://www.googletagmanager.com/gtm.js?id=' + encodeURIComponent(gtmId);
    head.appendChild(gtmScript);
  }

  function updateConsentV2() {
    var consentData = {
      ad_storage: 'granted',
      analytics_storage: 'granted',
      ad_user_data: 'granted',
      ad_personalization: 'granted',
      functionality_storage: 'granted',
      security_storage: 'granted',
    };
    if (typeof window.gtag === 'function') {
      window.gtag('consent', 'update', consentData);
      return;
    }
    if (Array.isArray(window.dataLayer)) {
      window.dataLayer.push({ event: 'iag_lgpd_consent_update', 'gtm.consent': consentData });
    }
  }

  function hideBanner(banner) {
    if (!banner) return;
    banner.style.display = 'none';
  }

  function handleAccept(options) {
    setCookie(COOKIE_NAME, 'accepted', COOKIE_EXPIRY_DAYS);
    if (Array.isArray(options.tags_after) && options.tags_after.length) {
      injectTags(options.tags_after);
    }
    if (options.gtm && options.useConsentModeV2) {
      updateConsentV2();
    }
    if (options._bannerInstance) {
      hideBanner(options._bannerInstance);
    }
  }

  function handleReject(options) {
    setCookie(COOKIE_NAME, 'rejected', COOKIE_EXPIRY_DAYS);
    if (options._bannerInstance) {
      hideBanner(options._bannerInstance);
    }
    if (options.reject_redirect_url) {
      window.location.href = options.reject_redirect_url;
    }
  }

  function buildBannerMarkup(options) {
    var wrapper = document.createElement('div');
    wrapper.className = 'iag-lgpd-overlay' + (options.banner_type === 'modal' ? ' iag-lgpd-modal-overlay' : '');
    wrapper.style.display = 'none';

    var content = document.createElement('div');
    content.className = 'iag-lgpd-banner iag-lgpd-' + options.banner_type;
    content.setAttribute('role', 'dialog');
    content.setAttribute('aria-live', 'polite');
    content.setAttribute('aria-label', 'Banner de cookies');

    var message = document.createElement('div');
    message.className = 'iag-lgpd-message';
    message.innerHTML = options.message;
    content.appendChild(message);

    var actions = document.createElement('div');
    actions.className = 'iag-lgpd-actions';

    if (options.policy_url) {
      var policyLink = document.createElement('a');
      policyLink.className = 'iag-lgpd-policy-btn';
      policyLink.href = options.policy_url;
      policyLink.target = '_blank';
      policyLink.rel = 'noopener noreferrer';
      policyLink.textContent = 'Política de Privacidade';
      actions.appendChild(policyLink);
    }

    if (options.show_reject_button) {
      var reject = document.createElement('button');
      reject.className = 'iag-lgpd-button iag-lgpd-button-reject';
      reject.type = 'button';
      reject.textContent = 'Recusar';
      reject.addEventListener('click', function () {
        handleReject(options);
      });
      actions.appendChild(reject);
    }

    var accept = document.createElement('button');
    accept.className = 'iag-lgpd-button iag-lgpd-button-accept';
    accept.type = 'button';
    accept.textContent = 'Aceitar';
    accept.addEventListener('click', function () {
      handleAccept(options);
    });
    actions.appendChild(accept);

    content.appendChild(actions);
    wrapper.appendChild(content);
    document.body.appendChild(wrapper);
    return wrapper;
  }

  function applyBannerStyles(options) {
    var css = '';
    css += '.iag-lgpd-overlay{position:fixed;inset:0;display:flex;justify-content:center;align-items:flex-end;z-index:2147483647;pointer-events:none;padding:0;margin:0;}';
    css += '.iag-lgpd-modal-overlay{align-items:center;background:rgba(0,0,0,.45);}';
    css += '.iag-lgpd-banner{pointer-events:auto;box-sizing:border-box;position:relative;font-family:inherit;font-size:inherit;color:' + options.banner_text_color + ';background:' + options.banner_bg_color + ';border-radius:12px;box-shadow:0 12px 40px rgba(0,0,0,.18);padding:18px;max-width:100%;width:100%;margin:12px;line-height:1.5;display:flex;flex-direction:column;gap:16px;}';
    css += '.iag-lgpd-banner *{font:inherit;color:inherit;}';
    css += '.iag-lgpd-bottom_bar{width:100%;max-width:900px;border-radius:0;margin:0 auto;flex-direction:row;justify-content:space-between;align-items:center;flex-wrap:wrap;}';
    css += '.iag-lgpd-bottom_bar .iag-lgpd-message{flex:1 1 60%;min-width:220px;margin-right:20px;}';
    css += '.iag-lgpd-bottom_bar .iag-lgpd-actions{flex:1 1 35%;justify-content:flex-end;}';
    css += '.iag-lgpd-floating_left,.iag-lgpd-floating_right{position:fixed;bottom:16px;max-width:360px;width:calc(100% - 32px);margin:0;}';
    css += '.iag-lgpd-floating_left{left:16px;}';
    css += '.iag-lgpd-floating_right{right:16px;}';
    css += '.iag-lgpd-floating_left .iag-lgpd-actions,.iag-lgpd-floating_right .iag-lgpd-actions{flex-direction:column;align-items:stretch;}';
    css += '.iag-lgpd-floating_left .iag-lgpd-button,.iag-lgpd-floating_left .iag-lgpd-policy-btn,.iag-lgpd-floating_right .iag-lgpd-button,.iag-lgpd-floating_right .iag-lgpd-policy-btn{width:100%;}';
    css += '.iag-lgpd-modal{max-width:600px;width:100%;border-radius:18px;margin:0;}';
    css += '.iag-lgpd-message{overflow-wrap:break-word;word-break:break-word;max-width:100%;}';
    css += '.iag-lgpd-actions{display:flex;flex-wrap:wrap;gap:10px;align-items:center;justify-content:flex-end;}';
    css += '.iag-lgpd-button,.iag-lgpd-policy-btn{border:none;border-radius:8px;padding:10px 16px;text-decoration:none;cursor:pointer;font:0.95rem inherit;transition:transform .15s ease,opacity .15s ease;}';
    css += '.iag-lgpd-button{background:' + options.btn_bg_color + ';color:' + options.btn_text_color + ';}';
    css += '.iag-lgpd-button:hover{opacity:.92;transform:translateY(-1px);}';
    css += '.iag-lgpd-policy-btn{background:transparent;color:' + options.banner_text_color + ';border:1px solid ' + options.banner_text_color + ';}';
    css += '.iag-lgpd-policy-btn:hover{opacity:.9;}';
    css += '@media (max-width:720px){.iag-lgpd-bottom_bar{flex-direction:column;}.iag-lgpd-bottom_bar .iag-lgpd-message{margin-right:0;}.iag-lgpd-bottom_bar .iag-lgpd-actions{flex-direction:column;align-items:stretch;justify-content:flex-start;width:100%;text-align:center;}.iag-lgpd-bottom_bar .iag-lgpd-button,.iag-lgpd-bottom_bar .iag-lgpd-policy-btn{width:100%;}.iag-lgpd-floating_left,.iag-lgpd-floating_right{left:16px;right:16px;width:auto;max-width:none;bottom:16px;}.iag-lgpd-actions{justify-content:center;}}';
    css += options.custom_css || '';
    injectStyles(css);
  }

  function initOptions(args) {
    var options = {};
    args = args || {};
    Object.keys(DEFAULTS).forEach(function (key) {
      options[key] = safeGet(args, key);
    });

    if (typeof options.banner_type !== 'string') {
      options.banner_type = DEFAULTS.banner_type;
    }
    if (['bottom_bar', 'floating_left', 'floating_right', 'modal'].indexOf(options.banner_type) === -1) {
      options.banner_type = DEFAULTS.banner_type;
    }
    options.tags_before = Array.isArray(options.tags_before) ? options.tags_before : DEFAULTS.tags_before;
    options.tags_after = Array.isArray(options.tags_after) ? options.tags_after : DEFAULTS.tags_after;
    options.show_reject_button = Boolean(options.show_reject_button);
    options.useConsentModeV2 = Boolean(options.useConsentModeV2);
    options.gtm = typeof options.gtm === 'string' ? options.gtm.trim() : '';
    options.message = typeof options.message === 'string' && options.message.trim() ? options.message : DEFAULTS.message;
    options.banner_bg_color = typeof options.banner_bg_color === 'string' && options.banner_bg_color.trim() ? options.banner_bg_color : DEFAULTS.banner_bg_color;
    options.banner_text_color = typeof options.banner_text_color === 'string' && options.banner_text_color.trim() ? options.banner_text_color : DEFAULTS.banner_text_color;
    options.btn_bg_color = typeof options.btn_bg_color === 'string' && options.btn_bg_color.trim() ? options.btn_bg_color : DEFAULTS.btn_bg_color;
    options.btn_text_color = typeof options.btn_text_color === 'string' && options.btn_text_color.trim() ? options.btn_text_color : DEFAULTS.btn_text_color;
    options.policy_url = typeof options.policy_url === 'string' && options.policy_url.trim() ? options.policy_url : DEFAULTS.policy_url;
    options.reject_redirect_url = typeof options.reject_redirect_url === 'string' && options.reject_redirect_url.trim() ? options.reject_redirect_url : DEFAULTS.reject_redirect_url;
    options.custom_css = typeof options.custom_css === 'string' ? options.custom_css : DEFAULTS.custom_css;
    return options;
  }

  function iagLGPDApp(args) {
    if (window.__iagLGPDAppHasRun) return;
    window.__iagLGPDAppHasRun = true;

    if (!isBannerVisible()) return;

    var options = initOptions(args);
    injectTags(options.tags_before);
    if (options.gtm) {
      injectGTM(options.gtm, options.useConsentModeV2);
    }
    applyBannerStyles(options);

    var createBanner = function () {
      if (!document.body) return;
      var banner = buildBannerMarkup(options);
      options._bannerInstance = banner;
      banner.style.display = 'flex';
    };

    if (document.body) {
      createBanner();
    } else {
      document.addEventListener('DOMContentLoaded', createBanner);
    }
  }

  window.iagLGPDApp = iagLGPDApp;
})(window, document);
